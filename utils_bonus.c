...
{
	ol->medimag = (ol->maximag - ol->minimag);
	if (pressed == ESC)
		eject(ol);
	else if (pressed == LEFTARROW)
	{
		ol->minreal += ol->medreal * 0.15;
		ol->maxreal += ol->medreal * 0.15;
	}
	...
	{
		ol->minimag += ol->medimag * 0.15;
		ol->maximag += ol->medimag * 0.15;
	}
	else if (pressed == UPARROW)
	{
		ol->minimag -= ol->medimag * 0.15;
		ol->maximag -= ol->medimag * 0.15;
	}
